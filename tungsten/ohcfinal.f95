...
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(Efvalue,Env,omeganv) REDUCTION(+:ohcEf)
DO i2=1,nefmesh
    ! Fermi energy value
        ...
        DO j=1,nkmesh
            DO k=1,nkmesh
                ...
                DO j2=1,18
                    ...
                    ohcEf = ohcEf + 2*fdirac(Env,Efvalue)*omeganv*SQRT(2.0)*(echarge**2/hbar)/lparam
                END DO
            END DO
        END DO
    END DO
...
filename = "ohclog_" // dates(7:8) // "-" // dates(5:6) // "-" // dates(1:4) // "_" // &
...
WRITE(12,*) "   Ef               OHC"
DO i2=1,nefmesh
    WRITE(12,*) Efmin + (Efmax-Efmin)*(i2-1)/(nefmesh-1), ohcresult(i2)
END DO
...
! orbital current operator (e.g. Jl(1,2,-,-,-) = (vx.Ly+Ly.vx)/2)
FUNCTION Jl(vind,lind,kb1,kb2,kb3,dk) RESULT(C)

    IMPLICIT NONE
    INTEGER,INTENT(IN):: vind, lind
    REAL,INTENT(IN):: kb1, kb2, kb3, dk
    COMPLEX,DIMENSION(18,18):: v, l 
    COMPLEX,DIMENSION(18,18):: C
...
FUNCTION omegamn(m,n,kb1,kb2,kb3,dk,W,VR) RESULT (C)
    ...
    matJ = MATMUL(Jl(2,3,kb1,kb2,kb3,dk),um)
